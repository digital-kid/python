...
def underline_it2(event):
	...
	if "underline" in current_tags:
		...
		TextArea.tag_add("underline","sel.first","sel.last")
def center_align():
	center_font=font.Font(TextArea,TextArea.cget("font"))
	center_font.configure()
	TextArea.tag_configure("center",font=center_font,justify='center')
	current_tags=TextArea.tag_names("sel.first")
...
def left_align():
	left_font=font.Font(TextArea,TextArea.cget("font"))
	left_font.configure()
	TextArea.tag_configure("left",font=left_font,justify='left')
	current_tags=TextArea.tag_names("sel.first")
...
def right_align():
	right_font=font.Font(TextArea,TextArea.cget("font"))
	right_font.configure()
	TextArea.tag_configure("right",font=right_font,justify='right')
	current_tags=TextArea.tag_names("sel.first")
...
m.add_command(label="      Underline      CTRL+U",command=underline_it)
...
btnleft=Button(toolbar,image=left_icon,command=left_align)
btnleft.grid(column=2,row=2,padx=5)
btncenter=Button(toolbar,image=center_icon,command=center_align)
btncenter.grid(column=3,row=2,padx=5)
btnright=Button(toolbar,image=right_icon,command=right_align)
...
underlinebtn=Button(toolbar,image=underline_icon,command=underline_it)
...
root.bind("<Control-u>",underline_it2)
